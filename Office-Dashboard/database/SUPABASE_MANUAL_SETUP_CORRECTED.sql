...
CREATE TYPE "PhoneStatus" AS ENUM ('ACTIVE', 'INACTIVE', 'ARCHIVED');
CREATE TYPE "AccountStatus" AS ENUM ('ACTIVE', 'INACTIVE', 'LOGIN_ISSUE', 'SUSPENDED', 'UNKNOWN');
CREATE TYPE "RecoveryMethodType" AS ENUM ('EMAIL', 'PHONE');
CREATE TYPE "RelationshipType" AS ENUM ('GENERAL', 'LOGIN', 'RECOVERY');

-- ============================================================
-- PHASE 2 ENUMS
-- ============================================================

CREATE TYPE "DeviceStatus" AS ENUM ('ONLINE', 'OFFLINE', 'UNKNOWN', 'DISABLED');
CREATE TYPE "WhatsappSessionStatus" AS ENUM ('SETUP_REQUIRED', 'LINKING', 'LINKED', 'RELOGIN_REQUIRED', 'DISABLED', 'ERROR', 'UNKNOWN');
...
CREATE TABLE platform_accounts (
    ...
    account_status "AccountStatus" NOT NULL DEFAULT 'UNKNOWN',
    notes TEXT,
    created_by UUID NOT NULL REFERENCES app_users(id),
    updated_by UUID NOT NULL REFERENCES app_users(id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE phone_account_links (
    ...
    relationship_type "RelationshipType" NOT NULL DEFAULT 'GENERAL',
...
CREATE TABLE account_recovery_methods (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    platform_account_id UUID NOT NULL REFERENCES platform_accounts(id) ON DELETE CASCADE,
    method_type "RecoveryMethodType" NOT NULL,
...
CREATE TABLE whatsapp_sessions (
    ...
    status "WhatsappSessionStatus" NOT NULL DEFAULT 'SETUP_REQUIRED',
