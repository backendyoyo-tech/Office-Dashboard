...
export class PlatformAccountsController {
  ...
  async archive(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = uuidParamSchema.parse(req.params);
      // REPAIR D-009 / D-006 — DELETE is the canonical archive verb; an optional
      // optimistic-locking version may be supplied in the body or query string.
      const raw = (req.body && (req.body as any).version !== undefined)
        ? (req.body as any).version
        : (req.query as any).version;
      const version = raw === undefined || raw === null || raw === '' ? undefined : Math.trunc(Number(raw)) || undefined;
      const account = await platformAccountsService.archive(id, req, version);
      res.json(account);
    } catch (err) {
      next(err);
    }
