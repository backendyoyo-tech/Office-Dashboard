...
export enum ErrorCode {
  ...
  LAUNCH_TICKET_INVALID = 'LAUNCH_TICKET_INVALID',
  ...
  // Phase 1+2 repair additions
  // D-006 — optimistic locking / stale write detection (FR-018)
  CONCURRENCY_CONFLICT = 'CONCURRENCY_CONFLICT',
  // SEC-002 — account lockout after repeated failed logins
  ACCOUNT_LOCKED = 'ACCOUNT_LOCKED',
  // D-024 — first-time device registration requires admin/enrollment authorisation
  DEVICE_ENROLLMENT_REQUIRED = 'DEVICE_ENROLLMENT_REQUIRED',
  // SEC-003 — refresh tokens
  REFRESH_TOKEN_INVALID = 'REFRESH_TOKEN_INVALID',
  // D-019 — dedicated rate limits on sensitive endpoints
...
const ERROR_HTTP_STATUS: Record<ErrorCode, number> = {
  ...
  [ErrorCode.LAUNCH_TICKET_INVALID]: 403,
  [ErrorCode.LAUNCH_TICKET_EXPIRED]: 410,
  ...
  [ErrorCode.DEVICE_ENROLLMENT_REQUIRED]: 401,
