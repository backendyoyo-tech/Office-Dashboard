...
const AccountDetailPage: React.FC = () => {
  ...
  const archiveMutation = useMutation({
    mutationFn: () => platformAccountsApi.archive(id!, account!.version),
    onSuccess: () => {
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-xs text-gray-500">
                  Last updated: {account.credential.secretUpdatedAt ? new Date(account.credential.secretUpdatedAt).toLocaleString() : '—'}
                </p>
              </div>
            </div>
          ) : (
