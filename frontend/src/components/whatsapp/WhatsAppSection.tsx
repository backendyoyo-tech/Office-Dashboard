...
import { Smartphone, RefreshCw, Unlink, Settings, ExternalLink } from 'lucide-react';
...
export const WhatsAppSection: React.FC<WhatsAppSectionProps> = ({ phoneNumberId, session }) => {
  ...
  const openMutation = useMutation({
    mutationFn: (sessionId: string) => whatsappApi.openSession(sessionId),
    onSuccess: () => {
  ...
  const disableMutation = useMutation({
    mutationFn: (sessionId: string) => whatsappApi.disableSession(sessionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['phone-number', phoneNumberId] });
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        {canEdit && (
          <div className="flex items-center gap-2">
            ...
            {(session.status === WhatsAppSessionStatus.RELOGIN_REQUIRED ||
              ...
              <button
                onClick={() => reconnectMutation.mutate(session.id)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => disableMutation.mutate(session.id)}
